...
public class Q1Worker
        implements
        Callable<Qualify> {
    private static final Logger logger = LoggerFactory.getLogger( Q1Worker.class );
    
    public static final String  Q1_URL_SUFFIX      = "/gb/Qualify.asp";

    private final HtmlPage      q1Page;

    public Q1Worker(HtmlPage q1Page) {
        this.q1Page = q1Page;
    }
    ...
    public Qualify parsePage(HtmlPage q1Page) {
        logger.info( "Parsing qualify 1..." );

        HtmlTable table = q1Page.getFirstByXPath( "//th[contains(text(),'Qualify 1 lap data')]/ancestor::table" );
        if( table != null ) {
            Qualify q1 = new Qualify();
            q1.setNumber( 1 );
            ...
            try {
                ...
                lap.getSettings().setTyre( Tyre.determineTyre( cells.get( 8 ).getTextContent().trim() ) );
                q1.setRiskDescr( cells.get( 9 ).getTextContent().trim() );
            } catch ( Exception e ) {
                logger.error( "Error parsing q1 lap'" + row.asText() + "'", e );
            }
            logger.info( "Q1 parsed: " + q1 );
            return q1;
        } else {
            logger.error( "Unable to find Q1 data." );
            return null;
        }
    }
